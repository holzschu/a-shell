...
function! xolox#misc#perm#update(fname, contents)
  ...
  " iOS: don't use this because chown is forbidden, and calling 'stat' is
...
function! xolox#misc#perm#get(fname)
  ...
  if filereadable(pathname)
    ...
    " iOS: we use BSD stat, not GNU stat:
    let command = printf('stat -f %s %s', '%Su:%Sg:%Lp', shellescape(pathname))
