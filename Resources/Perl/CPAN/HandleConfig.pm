...
sub neatvalue {
    ...
    unless ($t) {
        $v =~ s/\\/\\\\/g;
		# iOS: replace occurences of $HOME with \$ENV{HOME} because HOME changes.
		my $w = $ENV{HOME};
		$v =~ s/$w/\\\$ENV\{HOME\}/g;
		# iOS: same with $APPDIR:
		$w = $ENV{APPDIR};
		$v =~ s/$w/\\\$ENV\{APPDIR\}/g;
        return "q[$v]";
    }
...
sub cpan_home_dir_candidates {
    ...
    my $dotcpan = $^O eq 'VMS' ? '_cpan' : 'Documents/.cpan';
...
sub load {
	...
	# iOS addition: replace `\$ENV{HOME}` with the value of $ENV{HOME}:
	$CPAN::Config->{build_dir} =~  s/\\\$ENV\{HOME\}/$ENV{HOME}/g;
	$CPAN::Config->{cpan_home} =~  s/\\\$ENV\{HOME\}/$ENV{HOME}/g;
	$CPAN::Config->{histfile} =~  s/\\\$ENV\{HOME\}/$ENV{HOME}/g;
	$CPAN::Config->{keep_source_where} =~  s/\\\$ENV\{HOME\}/$ENV{HOME}/g;
	$CPAN::Config->{prefs_dir} =~  s/\\\$ENV\{HOME\}/$ENV{HOME}/g;
	# iOS: same with $ENV{APPDIR}:
	$CPAN::Config->{curl} =~  s/\\\$ENV\{APPDIR\}/$ENV{APPDIR}/g;
	$CPAN::Config->{gzip} =~  s/\\\$ENV\{APPDIR\}/$ENV{APPDIR}/g;
	$CPAN::Config->{tar} =~  s/\\\$ENV\{APPDIR\}/$ENV{APPDIR}/g;
	$CPAN::Config->{make} =~  s/\\\$ENV\{APPDIR\}/$ENV{APPDIR}/g;
	$CPAN::Config->{make_install_make_command} =~  s/\\\$ENV\{APPDIR\}/$ENV{APPDIR}/g;
	# TODO: as new commands are added, add them to the list here.
